...
run "echo 'clear_env = no' >> ${PHP_FPM_POOL_CONF}"
...
if [ ! -f "/etc/php.d/opcache.ini" ]; then
	echo "Error, opcache config file not found in: /etc/php.d/opcache.ini"
	exit 1
fi
...
run "sed -i'' 's|^;*opcache.file_cache[[:space:]]*=.*$|opcache.file_cache = \"${PHP_OPCACHE_FILE_CACHE}\"|g' /etc/php.d/opcache.ini"
